#!/usr/bin/env python3
"""
benchmarks/run_principal_branch_control.py
-------------------------------------------
Ablation study: PrincipalBranchProtocol using ablation_principal.yaml.
"""
...
from arp_topology.protocols import PrincipalBranchProtocol
from arp_topology.metrics import recovery_time, chern_fidelity
from arp_topology.plotting import plot_recovery

CONFIG_PATH = os.path.join(os.path.dirname(__file__), '..', 'configs', 'ablation_principal.yaml')
...
def main():
    ...
    proto = PrincipalBranchProtocol(model=model, config=p)

    t_span = tuple(cfg['simulation']['t_span'])
    print(f"Running PrincipalBranchProtocol  t∈{t_span}  M₀={m['M']}")
    ...
    save_path = os.path.join(out_dir, 'principal_branch.png')
    plot_recovery(result, protocol_name='Principal Branch', save_path=save_path)
